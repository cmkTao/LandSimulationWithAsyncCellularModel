...
TiffDataWrite::~TiffDataWrite(void) { close(); }

void TiffDataWrite::close() {
    // write into data
    if (mpoDataset != NULL && mpData != NULL) {
        mpoDataset->RasterIO(GF_Write, 0, 0, mnCols, mnRows, mpData, mnCols, mnRows, mgDataType, mnBands, 0, 0, 0, 0);
...
bool TiffDataWrite::init(const char* _filename, int _rows, int _cols, int _bandnum, double _pGeoTransform[6],
    const char* _sProjectionRef, GDALDataType _datatype /*= GDT_Byte*/, double _dInvalidVal /*= 0.0f*/) {
    close();
    ...
    for (int i = 0; i < 6; i++) mpGeoTransform[i] = _pGeoTransform[i];
    ...
    mpoDataset = mpoDriver->Create(msFilename, mnCols, mnRows, mnBands, mgDataType, NULL);
    if (mpoDataset == NULL) {
...
bool TiffDataWrite::init(const char* _filename, TiffDataRead* pRead) {
    ...
    return init(_filename, pRead->rows(), pRead->cols(), pRead->bandnum(), pRead->geotransform(),
        pRead->projectionRef(), pRead->datatype(), pRead->invalidValue());
}

bool TiffDataWrite::init(const char* _filename, TiffDataRead* pRead, int bandnum, GDALDataType _datatype /*= GDT_Byte*/,
    ...
    return init(_filename, pRead->rows(), pRead->cols(), bandnum, pRead->geotransform(), pRead->projectionRef(),
        _datatype, _dInvalidVal);
}
...
GDALDriver* TiffDataWrite::poDriver() { return mpoDriver; }

GDALDataset* TiffDataWrite::poDataset() { return mpoDataset; }

size_t TiffDataWrite::rows() { return mnRows; }

size_t TiffDataWrite::cols() { return mnCols; }

size_t TiffDataWrite::bandnum() { return mnBands; }

size_t TiffDataWrite::datalength() { return mnDatalength; }

double TiffDataWrite::invalidValue() { return mdInvalidValue; }

unsigned char* TiffDataWrite::imgData() { return mpData; }

GDALDataType TiffDataWrite::datatype() { return mgDataType; }

double* TiffDataWrite::geotransform() { return mpGeoTransform; }

char* TiffDataWrite::projectionRef() { return msProjectionRef; }

size_t TiffDataWrite::perPixelSize() { return mnPerPixSize; }
