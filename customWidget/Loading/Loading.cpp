...
//********************************************** 设置部分 *************************************
//设置点的个数
void Loading::setDotCount(int count) { _count = count; }

//设置点的颜色
void Loading::setDotColor(const QColor &color) { _dotColor = color; }
...
//设置最大直径
void Loading::setMaxDiameter(float max) { _maxDiameter = max; }

//设置最小直径
void Loading::setMinDiameter(float min) { _minDiameter = min; }

void Loading::stop() { timer.stop(); }
//********************************************** 绘制部分 *************************************
//刷新界面
void Loading::refresh() { repaint(); }
...
void Loading::paintEvent(QPaintEvent *event) {
    ...
    QFont font("Microsoft YaHei", 5, 75);
    painter.setFont(font);
    painter.drawText(QPointF(half - _centerDistance + 10, half + 2), u8"运行中...");
}
...
void Loading::paintDot(QPainter &painter) {
    for (int i = 0; i < _count; i++) {
        ...
        //        QFont font("Microsoft YaHei", radii * 3, 75);
        //        painter.setFont(font);
        //        painter.drawText(QPointF(half, half), u8"12345");
    }
    _index++;
}
